...
settings, install a driver or service, or transmit device information. Windows
administrator approval is requested only when a detection starts because the
Bluetooth codec trace is restricted to elevated processes. A short quiet tone
helps Windows open a fresh A2DP stream.
...
5. Requests administrator approval only when detection starts
...
4. Administrator approval when starting codec detection
